...
export class EscrowService {
  ...
  async releaseMilestone(milestoneId: string, clientId: string) {
    ...
    await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      // Credit the freelancer's wallet pendingBalance
      const wallet = await tx.freelancerWallet.upsert({
        where: { userId: milestone.contract.freelancerId },
        update: {
          pendingBalance: { increment: milestone.amount },
        },
        create: {
          userId: milestone.contract.freelancerId,
          pendingBalance: milestone.amount,
          availableBalance: 0,
        },
      });

      // Log CREDIT_PENDING wallet transaction
      await tx.walletTransaction.create({
        data: {
          walletId: wallet.id,
          type: 'CREDIT_PENDING',
          amount: milestone.amount,
          note: 'Milestone payout pending — 3-day hold started',
          milestoneId,
        },
      });
